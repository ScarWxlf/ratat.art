			...
			<form method="POST" enctype="multipart/form-data" action="?/public_profile_picture">
				<div class="grid w-full max-w-xs items-center gap-1.5">
					<label
						...
						>Picture
						<input
							class="flex w-full rounded-md border border-black border-input bg-white text-sm text-gray-400 file:border-0 file:bg-black file:text-white file:text-sm file:font-medium"
							type="file"
							name="picture"
							id="picture"
						/>
				...
				<button class="mt-4 py-2 px-4 rounded-full bg-gray-400 w-4/12" type="submit">Upload</button>
			</form>
		</div>
		<form method="POST" action="?/public_profile">
